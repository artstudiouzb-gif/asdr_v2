...
migration_name = '2026_08_03_notification_center.sql'

if migration_name not in schema:
    ddl = '''
...
'''
    ...
    if marker not in schema:
        raise RuntimeError('Migration marker was not found in schema.sql')
    schema = schema.replace(marker, ddl + '\n' + marker, 1)

    old_tail = "    ('2026_08_02_social_scheduled_at.sql')\nON DUPLICATE KEY UPDATE filename = filename;"
    new_tail = "    ('2026_08_02_social_scheduled_at.sql'),\n    ('2026_08_03_notification_center.sql')\nON DUPLICATE KEY UPDATE filename = filename;"
    if old_tail not in schema:
        raise RuntimeError('Migration list tail was not found in schema.sql')
    schema = schema.replace(old_tail, new_tail, 1)
    schema_path.write_text(schema, encoding='utf-8')

# Restore the normal CI permissions/jobs after the one-time synchronizer.
ci_path = Path('.github/workflows/ci.yml')
ci = ci_path.read_text(encoding='utf-8')
ci = ci.replace('permissions:\n  contents: write\n', 'permissions:\n  contents: read\n', 1)
start = ci.find('  schema-sync:\n')
end = ci.find('  test:\n', start)
if start != -1 and end != -1:
    ci = ci[:start] + ci[end:]
ci_path.write_text(ci, encoding='utf-8')

Path('.github/workflows/diagnose-notification-tests.yml').unlink(missing_ok=True)
Path('notification-test-output.txt').unlink(missing_ok=True)
