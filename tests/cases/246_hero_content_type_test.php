...
test('Стили обложки: служебные детали прячутся по наведению, кнопка остановки — нет', function () {
    $css = (string) file_get_contents(APP_ROOT . '/public/assets/css/blocks/hero.css');

    // Всё, что скрывается до наведения, обязано быть под `hover: hover`:
    // на сенсорном экране наводить нечем, и спрятанное там пропало бы совсем.
    // Гейтов несколько (стрелки и полоса объявлены рядом со своими правилами),
    // поэтому собираем все, а не первый попавшийся.
    assert_true(preg_match_all('/@media \(hover: hover\)\s*\{(.+?)\n\}/s', $css, $m) >= 1,
        'блока с гейтом по наведению нет');
    $gated = implode("\n", $m[1]);

    assert_true(strpos($gated, '.hero__arrow') !== false, 'стрелки проявляются по наведению');
    assert_true(strpos($gated, '.hero__progress') !== false, 'полоса времени проявляется по наведению');

    // WCAG 2.2.2: механизм остановки движущегося содержимого должен быть
    // доступен, а не появляться только у того, кто доведёт курсор до слайда.
    assert_true(strpos($gated, '.hero__playpause') === false,
        'кнопку остановки автопрокрутки прятать нельзя');

    // Полоса — деталь, а не содержание: короткая и приглушённая.
    assert_true(preg_match('/\.hero__progress\s*\{[^}]*width:\s*56px/', $css) === 1,
        'полоса времени фиксированной ширины, а не во всю навигацию');
    assert_true(preg_match('/\.hero__progress\s*\{[^}]*opacity:\s*var\(--hero-progress-shown\)/', $css) === 1,
        'видимая прозрачность взята из переменной, чтобы не разъехалась с возвратом после наведения');
});
