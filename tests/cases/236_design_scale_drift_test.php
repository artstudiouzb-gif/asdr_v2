...
const DESIGN_SCALE_CEILINGS = [
    // 73 -> 51: соседние размеры, отличавшиеся меньше чем на 2 %, схлопнуты
    // к более употребимому соседу (см. план, раздел 3.1). Потолок опущен по
    // факту чистки — так он и должен двигаться.
    'font-size' => 51,
    'box-shadow' => 161,
    'border-radius' => 36,
    // 606 -> 610: сброс движения по системной настройке
    // (@media prefers-reduced-motion в a11y.css) без !important не работает —
    // он объявлен селектором «*», а компонентные правила бьют его
    // специфичностью. Это ровно тот случай, когда приоритет оправдан.
    // Порог поднят осознанно и с объяснением; «поднять, чтобы прошло» —
    // не повод, см. комментарий выше.
    '!important' => 610,
];
