...
const DESIGN_SCALE_CEILINGS = [
    ...
    'box-shadow' => 161,
    'border-radius' => 36,
    // 610 -> 243. Из темы снято 335 приоритетов: каждый снимали и сверяли
    // вычисленные стили всех элементов на 9 страницах в двух ширинах и двух
    // темах — возвращали только те, без которых что-то менялось (см. план,
    // раздел 3.1). Оставшиеся 243 — четыре законных случая: настройка из
    // админки обязана перебить тему, состояние (:hover, прозрачная шапка)
    // обязано перебить покой, сброс движения по prefers-reduced-motion
    // объявлен селектором «*» и иначе проигрывает компонентам по
    // специфичности, и запрет перехода у каруселей.
