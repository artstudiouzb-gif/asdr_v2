...
`currency_worker.php` нужен только если в конструкторе шапки стоит элемент
«Курсы валют». Публичная страница за курсами в сеть не ходит — она читает файл
кэша, а наполняет его этот воркер. Прежде курсы тянул рендер шапки, и на
промахе кэша посетитель ждал ответа cbu.uz; неудача при этом не запоминалась,
поэтому пока источник лежит, полный таймаут платил каждый запрос. Замерено:
733 мс против 37 мс на той же странице. Без cron сайт работает как обычно —
плашка покажет прежние курсы из кэша, а если его нет ни разу, не покажется
вовсе; при сохранении шапки с этим элементом курсы подтягиваются сразу.
