...
// Короткая метка раздела берётся из верхнего пункта действующего меню, а не
// хардкодится по slug. Так «О НАС», «ПРЕСС-ЦЕНТР» и другие разделы остаются
// корректными на всех языках и при переименовании меню в админке.
$editorialEyebrow = '';
if ($editorialPage) {
    $currentPath = parse_url(\App\Core\Locale::url('/' . ltrim((string) ($page['slug'] ?? ''), '/')), PHP_URL_PATH) ?: '/';
    ...
    $matchesMenuTree = static function (array $item) use (&$matchesMenuTree, $currentPath, $normalizePath, $lang): bool {
        if (!empty($item['is_divider'])) {
            return false;
        }
        $url = \App\Models\MenuItem::resolveUrl($item, $lang);
        if ($normalizePath($url) === $currentPath) {
            return true;
        }
        foreach ((array) ($item['children'] ?? []) as $child) {
            if ($matchesMenuTree($child)) {
                return true;
            }
        }
        return false;
    };
    foreach (\App\Models\MenuItem::activeForLang($lang) as $topItem) {
        if ($matchesMenuTree($topItem)) {
            $editorialEyebrow = trim((string) ($topItem['title'] ?? ''));
            break;
        }
    }
    if ($editorialEyebrow === '' && $ancestorTrail !== []) {
        $editorialEyebrow = trim((string) ($ancestorTrail[0]['title'] ?? ''));
    }
}
        ...
        <header class="content-pagehead content-pagehead--editorial">
            <div class="content-pagehead__inner">
                <div class="content-pagehead__copy">
                    <?php if ($editorialEyebrow !== ''): ?>
                        <div class="content-pagehead__eyebrow"><?= htmlspecialchars($editorialEyebrow, ENT_QUOTES) ?></div>
                    <?php endif; ?>
