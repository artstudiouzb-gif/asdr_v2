...
final class BlockSamples
{
    ...
    public static function all(?string $lang = null): array
    {
        ...
        return [
            ...
            'slider' => ['slides' => []],
            // Обложка страницы: один слайд без карусели — самый частый
            // случай. Кадр редактор подставляет свой, поэтому в образце
            // только текст и кнопка.
            'hero_v2' => ['slides' => [
                [
                    'eyebrow' => 'Раздел',
                    'title' => 'Заголовок *обложки*',
                    'subtitle' => 'Короткая строка о том, что на этой странице.',
                    'cta_text' => 'Подробнее',
                    'cta_url' => '#',
                ],
            ]],
