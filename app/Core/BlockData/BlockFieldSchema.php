...
final class BlockFieldSchema
{
    ...
    private const EXTRA = [
        ...
        'person_cards' => ['items' => []],
        'slider' => ['slides' => []],
        // Слайды обложки — репитер: схема репитеров не описывает.
        'hero_v2' => ['slides' => []],
        'counters' => ['items' => []],
        'cards_grid' => ['items' => []],
        'media_gallery' => ['items' => []],
    ...
    public static function all(): array
    {
        return self::$cache ??= [
            ...
            'hero_v2' => [
                'height' => Field::enum('Высота', [
                    'compact' => 'Компактная — заголовок и кнопка',
                    'regular' => 'Обычная',
                    'tall' => 'Высокая',
                    'full' => 'Во весь экран',
                ], 'regular', 'Высота на телефоне выводится из выбранной: узкий экран получает свою пропорцию, отдельной настройки не требуется.'),
                'text_position' => Field::enum('Положение текста', [
                    'left' => 'Слева',
                    'center' => 'По центру',
                    'right' => 'Справа',
                ], 'left'),
                'bg' => Field::enum('Фон', [
                    'navy' => 'Фирменный тёмно-синий',
                    'dark' => 'Тёмный',
                    'light' => 'Светлый',
                    'custom' => 'Свой цвет',
                ], 'navy', 'Виден там, где нет фотографии: по краям кадра и пока снимок грузится.'),
                'bg_color' => Field::color('Свой цвет фона', '#0b1a30')
                    ->onlyWhen('bg', ['custom']),
                'text_scheme' => Field::enum('Цвет текста', [
                    'auto' => 'Авто — по фону и наложению',
                    'light' => 'Светлый',
                    'dark' => 'Тёмный',
                ], 'auto', 'На фотографии цвет текста решает читаемость, а не палитра: «авто» выбирает его по плотности и светлоте наложения.'),
                'accent' => Field::color('Цвет основной кнопки', '#173a63', 'Пусто — акцент сайта из раздела «Дизайн». Цвет надписи считается по контрасту с заливкой, отдельной настройки не требуется.'),
                'overlay' => Field::enum('Наложение на кадр', [
                    'gradient' => 'Градиент',
                    'solid' => 'Сплошное',
                    'none' => 'Нет',
                ], 'gradient', 'Тёмное наложение затемняет кадр, светлое осветляет. Направление градиента считается от положения текста: гуще там, где читают.'),
                'overlay_color' => Field::color('Цвет наложения', '#0b1a30')
                    ->onlyWhen('overlay', ['gradient', 'solid']),
                'overlay_opacity' => Field::int('Плотность наложения, %', 0, 100, 45)
                    ->onlyWhen('overlay', ['gradient', 'solid']),
                'autoplay' => Field::int(
                    'Автопрокрутка, секунд (0 — выключена)',
                    0,
                    30,
                    0,
                    'Пауза при наведении и после действия посетителя, возобновление через десять секунд и отсутствие автопрокрутки на телефоне не настраиваются — это правильное поведение карусели, а не выбор редактора.'
                ),
            ],
            'person_cards' => [
                'title' => Field::text('Заголовок, показываемый на сайте')->named('title_field'),
