...
- **Мониторинг «Oʻzbekiston — 2030»** (`u30_report`) — готовый отчёт одним
  блоком, без полей: содержимое правится в `templates/blocks/u30_report.php`.
  Диаграммы рисует `public/assets/js/blocks/u30_report.js` по данным внутри
  скрипта; стили — `public/assets/css/blocks/u30-report.css`, всё в области
  `#uz2030-report` (двух таких блоков на странице быть не должно). Тяжёлые
  CSS/JS подключаются только на страницах с блоком — через
  `AssetCollector::CSS_MAP`/`JS_MAP` (renderStyles выводится в `<head>`;
  так же можно подключать ассеты и другим объёмным блокам).
