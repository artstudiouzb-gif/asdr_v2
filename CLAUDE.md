...
```bash
# Полный прогон: 1140 сценариев; без TEST_DB_* 181 DB-сценарий пропускается
...
- **Оргструктура** (`org_structure`): совет над руководителем, органы сбоку,
  ...
  Построчная разметка списков: `Название | /url` — ссылка, `* Название` —
  акцент (проектный офис), `- Название` — вложенная группа. Число колонок ряда
